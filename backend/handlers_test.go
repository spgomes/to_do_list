...
func TestAPIListsFullFlow(t *testing.T) {
	...
	protected.HandleFunc("POST /api/lists/{id}/todos", handleCreateTodoInList(db))
	mux.Handle("/api/todos", jwtMiddleware(protected))
	mux.Handle("/api/todos/", jwtMiddleware(protected))
...
// --- CreateTodoInList handler tests ---

func TestHandleCreateTodoInList_Success(t *testing.T) {
	db := setupTestDB(t)
	user := createTestUser(t, db, "user@test.com", "hash")
	list, _ := CreateList(db, "work", "#F8BBD9", user.ID)

	body := `{"title":"task in list"}`
	req := httptest.NewRequest(http.MethodPost, "/api/lists/"+strconv.FormatInt(list.ID, 10)+"/todos", bytes.NewBufferString(body))
	req.SetPathValue("id", strconv.FormatInt(list.ID, 10))
	req = injectUserID(req, user.ID)
	w := httptest.NewRecorder()

	handleCreateTodoInList(db)(w, req)
	...
	if todo.Title != "task in list" {
		t.Errorf("expected title 'task in list', got '%s'", todo.Title)
	}
	if todo.ID == 0 {
		t.Error("expected non-zero ID")
	}
	if len(todo.Lists) != 1 || todo.Lists[0].ID != list.ID {
		t.Errorf("expected todo to be associated with list %d, got lists %v", list.ID, todo.Lists)
	}
}

func TestHandleCreateTodoInList_ListNotFound(t *testing.T) {
	db := setupTestDB(t)
	user := createTestUser(t, db, "user@test.com", "hash")

	body := `{"title":"task"}`
	req := httptest.NewRequest(http.MethodPost, "/api/lists/999/todos", bytes.NewBufferString(body))
	req.SetPathValue("id", "999")
	req = injectUserID(req, user.ID)
	w := httptest.NewRecorder()
...
func TestHandleCreateTodoInList_EmptyTitle(t *testing.T) {
	db := setupTestDB(t)
	user := createTestUser(t, db, "user@test.com", "hash")
	list, _ := CreateList(db, "work", "#F8BBD9", user.ID)
